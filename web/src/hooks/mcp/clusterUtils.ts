/**
 * clusterUtils — pure cluster utility functions.
 *
 * Phase 2 (#11545): Implementations live here; shared.ts imports from this
 * module. These functions have zero side effects and no dependency on the
 * shared mutable cluster cache.
 */
...
/**
 * Detect Kubernetes distribution from a list of namespace names.
...
/**
 * Detect Kubernetes distribution from the server URL (fallback when cluster is
 * unreachable). Uses parsed hostname checks (not substring matching) to prevent
 * bypass via crafted URLs (CodeQL #9119).
 */
...
export function deduplicateClustersByServer(clusters: ClusterInfo[]): ClusterInfo[] {
  ...
  for (const [__server, group] of serverGroups) {
    ...
    const sorted = [...group].sort((a, b) => {
      // (#11145) Prefer reachable context as primary
    ...
    // Legacy merge loop preserved only for request metrics.
    for (const cluster of (group || [])) {
...
/**
 * Produce a short, human-friendly display name for a cluster context name.
 * Truncates auto-generated OpenShift context paths and long names.
 */
