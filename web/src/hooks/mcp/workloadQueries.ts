...
import {
  ...
  podIssuesCache,
  setPodIssuesCache,
  deploymentIssuesCache,
  setDeploymentIssuesCache,
  deploymentsCache,
  setDeploymentsCache,
...
export function usePods(cluster?: string, namespace?: string, sortBy: 'restarts' | 'name' = 'restarts', limit = 10): UsePodsResult {
  ...
  const refetch = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const now = new Date()
      setPodsCache({ data: sortedPods, timestamp: now, key: cacheKey })
      savePodsCacheToStorage()
...
export function useAllPods(cluster?: string, namespace?: string, forceLive = false): UseAllPodsResult {
  ...
  const refetch = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const now = new Date()
      setPodsCache({ data: allPods, timestamp: now, key: cacheKey })
      savePodsCacheToStorage()
  ...
  const refetch = useCallback(async (silent = false) => {
    ...
    if (cluster && !isAgentUnavailable()) {
      try {
        ...
        const now = new Date()
        setPodIssuesCache({ data: safePodIssues, timestamp: now, key: cacheKey })
    ...
    try {
      ...
      const now = new Date()
      setPodIssuesCache({ data: allIssues, timestamp: now, key: cacheKey })
      setIssues(allIssues)
      setError(null)
      setLastUpdated(now)
      setConsecutiveFailures(0)
...
export function useDeploymentIssues(cluster?: string, namespace?: string): UseDeploymentIssuesResult {
  ...
  const refetch = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const now = new Date()
      setDeploymentIssuesCache({ data: allIssues, timestamp: now, key: cacheKey })
      setIssues(allIssues)
      setError(null)
      setLastUpdated(now)
      setConsecutiveFailures(0)
...
export function useDeployments(cluster?: string, namespace?: string): UseDeploymentsResult {
  ...
  const refetch = useCallback(async (silent = false) => {
    ...
    if (cluster && !isAgentUnavailable() && LOCAL_AGENT_HTTP_URL) {
      try {
        ...
        if (response.ok) {
          ...
          const now = new Date()
          // Update cache
          setDeploymentsCache({ data: deployData, timestamp: now, key: cacheKey })
    ...
    if (cluster && !isAgentUnavailable()) {
      try {
        ...
        if (deployData && deployData.length >= 0) {
          ...
          const now = new Date()
          setDeploymentsCache({ data: enriched, timestamp: now, key: cacheKey })
    ...
    try {
      ...
      setConsecutiveFailures(0)
      setLastRefresh(now)
      setDeploymentsCache({ data: newDeployments, timestamp: now, key: cacheKey })
    } catch (err: unknown) {
