...
const controlStatusColor = (status: string) => {
  switch (status) {
    case 'satisfied': return 'text-green-600 dark:text-green-400'
    case 'partially_satisfied': return 'text-yellow-600 dark:text-yellow-400'
...
const milestoneStatusBadge = (status: string) => {
  switch (status) {
    case 'open': return <span className="px-2 py-0.5 rounded text-xs font-semibold bg-red-500/20 text-red-600 dark:text-red-400 border border-red-500/30">Open</span>
    case 'delayed': return <span className="px-2 py-0.5 rounded text-xs font-semibold bg-yellow-500/20 text-yellow-600 dark:text-yellow-400 border border-yellow-500/30">Delayed</span>
    case 'closed': return <span className="px-2 py-0.5 rounded text-xs font-semibold bg-green-500/20 text-green-600 dark:text-green-400 border border-green-500/30">Closed</span>
    case 'in_progress': return <span className="px-2 py-0.5 rounded text-xs font-semibold bg-blue-500/20 text-blue-600 dark:text-blue-400 border border-blue-500/30">In Progress</span>
    default: return <span className="px-2 py-0.5 rounded text-xs font-semibold bg-secondary text-secondary-foreground border border-border">{status}</span>
  }
}
      ...
      {score && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="bg-card rounded-lg p-4 border border-border shadow-sm">
            ...
            <p className="text-2xl font-bold text-foreground capitalize">{milestoneStatusBadge(score.authorization_status)}</p>
          </div>
          <div className="bg-card rounded-lg p-4 border border-green-600/40 dark:border-green-500/30 shadow-sm">
            <p className="text-sm text-muted-foreground">Satisfied</p>
            <p className="text-2xl font-bold text-green-600 dark:text-green-400">{score.controls_satisfied}</p>
          </div>
          <div className="bg-card rounded-lg p-4 border border-yellow-600/40 dark:border-yellow-500/30 shadow-sm">
            <p className="text-sm text-muted-foreground">Partial</p>
            <p className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">{score.controls_partially_satisfied}</p>
          </div>
          <div className="bg-card rounded-lg p-4 border border-blue-600/40 dark:border-blue-500/30 shadow-sm">
            <p className="text-sm text-muted-foreground">Impact Level</p>
            <p className="text-2xl font-bold text-blue-600 dark:text-blue-400 capitalize">{score.impact_level}</p>
          </div>
        </div>
      )}
          ...
          <div className="flex gap-2 flex-wrap">
            {['all', 'satisfied', 'partially_satisfied', 'planned'].map(s => (
              <button
                ...
                className={`px-3 py-1 rounded text-xs font-medium border transition-colors ${statusFilter === s ? 'bg-blue-600 dark:bg-blue-500 text-white border-blue-600 dark:border-blue-500 shadow-sm' : 'bg-secondary text-foreground border-border hover:bg-secondary/80'}`}
              ...
              <tbody>
                {filteredControls.map(c => (
                  <tr key={c.id} className="border-b border-border/50 hover:bg-muted/30">
                    <td className="p-3 font-mono text-blue-600 dark:text-blue-300">{c.id}</td>
                    <td className="p-3 text-foreground">{c.name}</td>
                    <td className="p-3"><span className="px-2 py-0.5 rounded bg-secondary text-secondary-foreground text-xs">{c.family}</span></td>
                    <td className="p-3">
            ...
            <tbody>
              {poams.map(p => (
                <tr key={p.id} className="border-b border-border/50 hover:bg-muted/30">
                  <td className="p-3 font-mono text-blue-600 dark:text-blue-300">{p.id}</td>
                  <td className="p-3 text-muted-foreground">{p.control_id}</td>
                  <td className="p-3 text-foreground">{p.title}</td>
                  <td className="p-3">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                      p.risk_level === 'high' ? 'bg-red-500/20 text-red-600 dark:text-red-400' :
                      p.risk_level === 'moderate' ? 'bg-yellow-500/20 text-yellow-600 dark:text-yellow-400' :
                  ...
                  <td className="p-3">
                    {p.vendor_dependency
                      ? <span className="text-yellow-600 dark:text-yellow-400 text-xs font-medium">Yes</span>
        ...
        <div className="space-y-4">
          <div className="bg-card rounded-lg border border-border p-6 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="mt-1">{milestoneStatusBadge(score.authorization_status)}</div>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Overall Score</p>
              ...
              <div>
                <p className="text-sm text-muted-foreground">Impact Level</p>
                <p className="text-xl font-bold text-blue-600 dark:text-blue-400 capitalize">{score.impact_level}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Open POAMs</p>
                <p className="text-xl font-bold text-red-600 dark:text-red-400">{score.poams_open}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Last Evaluated</p>
                <p className="text-sm text-muted-foreground">{new Date(score.evaluated_at).toLocaleString()}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-card rounded-lg border border-border p-6 shadow-sm">
            ...
            <div className="space-y-3">
              {[
                { label: 'Satisfied', count: score.controls_satisfied, color: 'bg-green-500', textColor: 'text-green-600 dark:text-green-400' },
                { label: 'Partially Satisfied', count: score.controls_partially_satisfied, color: 'bg-yellow-500', textColor: 'text-yellow-600 dark:text-yellow-400' },
                { label: 'Planned', count: score.controls_planned, color: 'bg-blue-500', textColor: 'text-blue-600 dark:text-blue-400' },
              ].map(item => (
