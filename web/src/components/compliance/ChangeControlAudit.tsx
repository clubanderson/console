  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'changes' && (
        <div className="space-y-3">
          ...
          {(filteredChanges || []).length === 0 ? (
            <p className="text-muted-foreground text-sm text-center py-8">No changes found matching the selected filter</p>
          ) : (filteredChanges || []).map(change => {
            const approval = APPROVAL_STYLES[change.approval_status] ?? APPROVAL_STYLES.pending
            return (
              <div key={change.id} className="rounded-lg border border-border/50 bg-card p-4">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                  ...
                  {change.diff_summary && <code className="bg-secondary/50 px-1 rounded">{change.diff_summary}</code>}
                </div>
              </div>
            )
          })}
      ...
      {activeTab === 'violations' && (
        <div className="space-y-2">
          {(violations || []).length === 0 ? <p className="text-muted-foreground text-sm text-center py-8">No policy violations detected</p> : (violations || []).map(v => (
            <div key={v.id} className="rounded-lg border border-border/50 bg-card/50 p-3">
      ...
      {activeTab === 'policies' && (
        <div className="space-y-2">
          {(policies || []).map(p => (
            <div key={p.id} className="rounded-lg border border-border/50 bg-card/50 p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
