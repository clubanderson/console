...
const SEVERITY_STYLES: Record<string, string> = {
  ...
  low:      'bg-zinc-500/20 text-muted-foreground border-border',
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b border-border pb-0">
        {(['violations', 'principals', 'rules'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors ${activeTab === tab ? 'bg-secondary text-foreground border-b-2 border-indigo-400' : 'text-muted-foreground hover:text-foreground hover:bg-card'}`}>
      ...
      {activeTab === 'violations' && (
        <div className="space-y-3">
          ...
          {filteredViolations.length === 0 ? <p className="text-muted-foreground text-sm text-center py-8">No violations found</p> : filteredViolations.map(v => (
            <div key={v.id} className="rounded-lg border border-border/50 bg-card p-4">
      ...
      {activeTab === 'principals' && (
        <div className="space-y-2">
          {principals.map(p => {
            ...
            return (
              <div key={p.name} className={`rounded-lg border p-3 ${hasViolation ? 'border-red-500/30 bg-red-500/5' : 'border-border/50 bg-card/50'}`}>
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {(p.roles || []).map(r => <span key={r} className="text-xs bg-secondary px-1.5 py-0.5 rounded text-muted-foreground">{r}</span>)}
                </div>
      ...
      {activeTab === 'rules' && (
        <div className="space-y-2">
          {rules.map(r => (
            <div key={r.id} className="rounded-lg border border-border/50 bg-card/50 p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
              ...
              <div className="flex gap-2 text-xs">
                <span className="bg-secondary px-2 py-0.5 rounded text-muted-foreground">{r.role_a}</span>
                <XCircle className="w-4 h-4 text-red-400" />
                <span className="bg-secondary px-2 py-0.5 rounded text-muted-foreground">{r.role_b}</span>
              </div>
            </div>
          ))}
        </div>
