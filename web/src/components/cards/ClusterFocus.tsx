    ...
    <div className="h-full flex flex-col min-h-card content-loaded overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div
          ...
          onKeyDown={(e) => { if ((e.key === 'Enter' || e.key === ' ') && cluster) { e.preventDefault(); drillToCluster(cluster.name, { healthy: cluster.healthy, nodeCount: cluster.nodeCount, podCount: cluster.podCount, cpuCores: cluster.cpuCores, server: cluster.server }) } }}
          role="button"
          tabIndex={0}
        >
      ...
      <div className="space-y-2">
        <div
          ...
          onKeyDown={(e) => {
            if ((e.key === 'Enter' || e.key === ' ') && podIssues.length > 0) {
              e.preventDefault()
              const issue = podIssues[0]
              drillToPod(clusterName, issue.namespace, issue.name, {
                status: issue.status,
        ...
        <div
          ...
          onKeyDown={(e) => {
            if ((e.key === 'Enter' || e.key === ' ') && deploymentIssues.length > 0) {
              e.preventDefault()
              const issue = deploymentIssues[0]
              drillToDeployment(clusterName, issue.namespace, issue.name, {
                replicas: issue.replicas,
