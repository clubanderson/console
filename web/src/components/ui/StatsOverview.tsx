      ...
      {effectiveMode === 'sparkline' && hasEnoughHistory && !isNaN(numericValue) ? (
        <>
          <div className="flex items-end justify-between gap-2">
            <div className={`text-2xl font-bold ${isLoading ? 'text-muted-foreground/30' : valueColor}`} data-testid={`stat-block-${block.id}-count`}>
              {displayValue}
            </div>
        ...
        <>
          ...
          <div className="relative">
            <div className={`text-3xl font-bold ${numericValue > 0 ? 'text-white drop-shadow-xs' : valueColor}`} data-testid={`stat-block-${block.id}-count`}>{displayValue}</div>
        ...
        <>
          <div className={`text-3xl font-bold ${isLoading ? 'text-muted-foreground/30' : valueColor}`} data-testid={`stat-block-${block.id}-count`}>{displayValue}</div>
